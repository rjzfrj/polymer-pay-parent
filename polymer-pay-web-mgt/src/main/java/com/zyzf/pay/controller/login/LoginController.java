 ...
 */
package com.zyzf.pay.controller.login;
...
import com.zyzf.pay.common.core.dwz.DWZ;
...
import com.zyzf.pay.permission.exception.PermissionException;
...
public class LoginController extends BaseController {
	...
	public String index(HttpServletRequest req, Model model) {
		ZyppOperator ZyppOperator = (ZyppOperator) this.getSession().getAttribute("ZyppOperator");
		try {
			String tree = this.buildOperatorPermissionMenu(ZyppOperator);
	 ...
	 * @throws Exception
	 */
	private String buildOperatorPermissionMenu(ZyppOperator ZyppOperator) throws PermissionException {
		// 根据用户ID得到该用户的所有角色拼成的字符串
		String roleIds = ZyppOperatorRoleService.getRoleIdsByOperatorId(ZyppOperator.getId());
		if (StringUtils.isBlank(roleIds)) {
			LOG.error("==>用户[" + ZyppOperator.getLoginName() + "]没有配置对应的权限角色");
	...
	public String buildPermissionTree(String roleIds) throws PermissionException {
		List treeData = null;
		try {
			treeData = ZyppMenuService.listByRoleIds(roleIds);
